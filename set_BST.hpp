...
namespace ft
{
	...
	class setRBtree
	{
		public:
			...
			typedef typename ft::RBtree_iterator_const<value_type>				iterator;
		...
		public:
			setRBtree(void) : _node_alloc(allocator_type())
			{
			...
			setRBtree(const value_type &val) : _comp(value_compare()), _node_alloc(allocator_type())
			{
				_nil =_node_alloc.allocate(1);
			...
			node_ptr find_key(const value_type & k) const
			{
				...
				while (node != _nil)
				{
					if (_comp(node->_value, k) == false && _comp(k, node->_value) == false)
						return (node);
					if (_comp(node->_value, k))
						node = node->_right;
					else if (_comp(k, node->_value))
						node = node->_left;
				}
			...
			iterator insert(node_ptr* root, const value_type &val)
			{
				...
				while ((*root) != _nil)
				{
					if (_comp((*root)->_value, val) == false && _comp(val, (*root)->_value) == false)
						return (iterator(insert_fix((*root)), _nil));
					if ((*root)->_left == _nil && !_comp((*root)->_value, val))
					{
						create_node(&((*root)->_left), val);
						(*root)->_left->_parent = (*root);
						return (iterator(insert_fix((*root)->_left), _nil));
					}
					if ((*root)->_right == _nil && _comp((*root)->_value, val))
					{
						create_node(&((*root)->_right), val);
						(*root)->_right->_parent = (*root);
						return (iterator(insert_fix((*root)->_right), _nil));
					}
					if (_comp((*root)->_value, val))
						root = &((*root)->_right);
					else if (_comp(val, (*root)->_value))
						root = &((*root)->_left);
				}
